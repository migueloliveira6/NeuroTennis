...
class TennisPredictor:
    ...
    def load_data(self):
        ...
        self.matches = pd.read_csv('/content/drive/MyDrive/Colab Notebooks/atp_chall_matches_2025_elo.csv',
    ...
    def save_model(self):
        ...
        # Detectar automaticamente o caminho correto
        if 'COLAB_GPU' in os.environ:
            model_path = '/content/drive/MyDrive/Colab Notebooks/models/'
        elif os.path.exists('D:/projetos/Tenis ML-AI/models/'):
    ...
    def load_saved_model(self):
        ...
        try:
            ...
            model_file = os.path.join(model_path, 'tennis_surface_elo_model_xgboost.pkl')
            scaler_file = os.path.join(model_path, 'tennis_surface_elo_scaler_xgboost.pkl')
            data_file = os.path.join(model_path, 'tennis_surface_elo_data_xgboost.pkl')
            
            missing_files = []
            for file_path, file_name in [(model_file, 'model'), (scaler_file, 'scaler'), (data_file, 'data')]:
                if not os.path.exists(file_path):
                    missing_files.append(file_name)
            
            if missing_files:
                print(f"Arquivos em falta: {missing_files}")
                return False
